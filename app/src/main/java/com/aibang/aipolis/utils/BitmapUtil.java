...
//        BitmapFactory.decodeFile(pathName, options);
    ...
    public static boolean saveBitmapFile(Bitmap bitmap, String filePath, String fileName) {
...
//            }
    ...
    public static Bitmap ratio(Bitmap image, float pixelW, float pixelH) {
        ...
        if (size >= 1024 * 1.5) {//判断如果图片大于1.5M,进行压缩避免在生成图片（BitmapFactory.decodeStream）时溢出
            Log.d("imgsize","大于1.5M,压缩40%");
            os.reset();//重置baos即清空baos
            image.compress(Bitmap.CompressFormat.JPEG, 40, os);//这里压缩50%，把压缩后的数据存放到baos中
        } else if (size >= 1024 && size < 1024 * 1.5) {
            Log.d("imgsize","大于1M,小于1.5,压缩50%");
            os.reset();//重置baos即清空baos
            image.compress(Bitmap.CompressFormat.JPEG, 50, os);//这里压缩40%，把压缩后的数据存放到baos中
        } else if (size > 400 && size < 1024) {
            Log.d("imgsize","大于400,小于1,压缩60%");
            os.reset();//重置baos即清空baos
            image.compress(Bitmap.CompressFormat.JPEG, 60, os);//这里压缩60%，把压缩后的数据存放到baos中
        }
//        Log.d("imgsize","小于于400,不压缩压缩");
