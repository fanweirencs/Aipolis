 ...
 */
public class MsgHelpCommentAdapter extends CommonAdapter<HelpComment> {

    private DisplayImageOptions options;
    private User user;

    public MsgHelpCommentAdapter(Context context, List<HelpComment> datas, int layoutId, User user) {
    ...
    @Override
    public void convert(final ViewHolder holder, final HelpComment comment) {
        ...
        if (comment.getBeiCommentUser() != user) {//评论
            ...
            holder.setText(R.id.id_life_content,
                     mContext.getString(R.string.comment_your) + comment.getHelp().getContent());
        }
